...
export function setupUserGithubRoutes(app: Express) {
  // Get user's GitHub token status (without exposing the token)
  ...
  app.post("/api/github/repos/:repoId/generate-blog", requireAuth, async (req, res) => {
    try {
      ...
      // Generate blog post from repository
  ...
  // Get user's GitHub repositories (using their personal token)
