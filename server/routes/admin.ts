/**
 * Comprehensive Admin Routes - Production Ready
 * Implements all missing admin features identified in the audit
 */
...
export function registerAdminRoutes(app: Express) {
  ...
  // Get single user with full activity details
  ...
  app.get('/api/admin/moderation/queue', requireAdmin, async (req, res) => {
    try {
      const type = req.query.type as string; // 'comments', 'posts', 'all'
      const status = req.query.status as string; // 'pending', 'flagged', 'all'
  ...
  // ===========================================
  // COMMENT MANAGEMENT ROUTES - ADMIN COMPLETE CONTROL
  // ===========================================
  ...
  // Edit any comment (admin privilege)
  ...
  // Block any comment (admin privilege)
