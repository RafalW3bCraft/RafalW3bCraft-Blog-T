...
export function PersonalBlogWorkspace() {
  ...
  // Fetch top 5 GitHub repositories with highest views for blog generation
  const { data: topRepos = [] } = useQuery({
    queryKey: ['top-github-repos'],
    queryFn: async () => {
      const res = await apiRequest('GET', '/api/github/projects');
      const repos = await res.json();
      // Sort by stars and watchers to simulate "highest views"
      return repos
  ...
  // Generate blog from repository mutation
  const generateBlogMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['user-drafts'] });
      // Switch to drafts tab to show the new blog post
      setActiveTab('drafts');
    },
    onError: (error: any) => {
      toast({
  ...
  const publishPostMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      // Reset current post after publishing
      setCurrentPost({
        title: '',
        content: '',
        excerpt: '',
